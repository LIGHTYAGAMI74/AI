...
export default function HomePage() {
  const [mounted, setMounted] = useState(false);
  const [showIntro, setShowIntro] = useState(false);
  // ⚡ ADDED: Get API URL from env
  const API_URL = process.env.NEXT_PUBLIC_API_URL;

  useEffect(() => {
    setMounted(true);

    // ⚡ ADDED: Simple Background Wake-up (Does not affect UI)
    const wakeUpServer = async () => {
      try {
        await fetch(`${API_URL}/`);
      } catch (err) {
        // Silent fail - just intended to trigger Render spin-up
      }
    };
    if (API_URL) wakeUpServer();
